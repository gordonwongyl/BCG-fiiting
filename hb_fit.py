...
class Hb(Lines):
    ...
    def plot_spe(self, ax_in, i, j):
        ...
        ax_in[0].plot(self.wlax[lranges], single(self.wlax[lranges], *popt), color='Orange')
        ax_in[0].axvline(x=popt[1], color='orange', label='central wavelength')
        #ax_in[0].axhline(y=np.median(fit_spec[lranges]), color='r', label='Median around line ranges')
        ax_in[0].set_title(f"Sample fit with single peak")
        ax_in[0].set_xlim((4825,4975))
        #ax_in[0].set_title(f"(i,j): ({j+1}, {i+1}), fitsnr = {fitsnr1}, {fitsnr2}, ratio = {popt[0]/popt[1]} rerr = {ratioerr} ")
        ax_in[0].legend(loc='upper right')
        ax_in[1].step(self.wlax, aC, where='mid', label='scaled template baseline (aC)')
