...
def sii_doublet(x, h1, h2, lmbda1, sig, c):
    return (h1/np.sqrt(2*np.pi)/sig)*np.exp(-(x-lmbda1)**2/2/sig**2) + \
           (h2/np.sqrt(2*np.pi)/sig)*np.exp(-(x-sii_wratio*lmbda1)**2/2/sig**2) + c
sii_doublet = np.vectorize(sii_doublet)

oiii_wratio = lines['OIII'][0][1]/lines['OIII'][0][0]
def oiii_doublet(x, h1, lmbda1, sig, c):
    return (h1/np.sqrt(2*np.pi)/sig/3)*np.exp(-(x-lmbda1)**2/2/sig**2) + \
           (h1/np.sqrt(2*np.pi)/sig)*np.exp(-(x-oiii_wratio*lmbda1)**2/2/sig**2) + c
oiii_doublet = np.vectorize(oiii_doublet)

oi_wratio = lines['OI'][0][1]/lines['OI'][0][0]
def oi_doublet(x, h1, h2, lmbda1, sig, c):
    return (h1/np.sqrt(2*np.pi)/sig)*np.exp(-(x-lmbda1)**2/2/sig**2) + \
           (h2/np.sqrt(2*np.pi)/sig)*np.exp(-(x-oi_wratio*lmbda1)**2/2/sig**2) + c
...
def hamod1(x, h_nii, h_ha, lmbda1, sig):
    return (h_nii/3/np.sqrt(2*np.pi)/sig)*np.exp(-(x-niil_wratio*lmbda1)**2/2/sig**2) + \
           (h_ha/np.sqrt(2*np.pi)/sig)*np.exp(-(x-lmbda1)**2/2/sig**2) + \
           (h_nii/np.sqrt(2*np.pi)/sig)*np.exp(-(x-niir_wratio*lmbda1)**2/2/sig**2) 
hamod1 = np.vectorize(hamod1)

def hamod2(x, h_nii1, h_ha1, lmbda1, sig1, h_nii2, h_ha2, lmbda2, sig2):
    return (h_nii1/3/np.sqrt(2*np.pi)/sig1)*np.exp(-(x-niil_wratio*lmbda1)**2/2/sig1**2) + \
           (h_ha1/np.sqrt(2*np.pi)/sig1)*np.exp(-(x-lmbda1)**2/2/sig1**2) + \
           (h_nii1/np.sqrt(2*np.pi)/sig1)*np.exp(-(x-niir_wratio*lmbda1)**2/2/sig1**2) \
           + \
           (h_nii2/3/np.sqrt(2*np.pi)/sig2)*np.exp(-(x-niil_wratio*lmbda2)**2/2/sig2**2) + \
           (h_ha2/np.sqrt(2*np.pi)/sig2)*np.exp(-(x-lmbda2)**2/2/sig2**2) + \
           (h_nii2/np.sqrt(2*np.pi)/sig2)*np.exp(-(x-niir_wratio*lmbda2)**2/2/sig2**2) 
...
WLAX = 4749.81005859 + np.arange(3681) * 1.25
class Lines:
    ...
    def get_slice(self, arr_in: NDArray) -> NDArray:
        mask = (self.cranges[0] < WLAX) & (WLAX < self.cranges[1])
        return arr_in[mask]
